    ...
    protected function initDefaultButtons()
    {
        if (!isset($this->buttons['view'])) {
            $this->buttons['view'] = function ($url, $model, $key) {
                $options = array_merge([
                    'title' => Yii::t('yii', 'Εμφάνιση'),
                    'aria-label' => Yii::t('yii', 'Εμφάνιση'),
                    'data-pjax' => '0',
                ], $this->buttonOptions);
        ...
        if (!isset($this->buttons['update'])) {
            $this->buttons['update'] = function ($url, $model, $key) {
                $options = array_merge([
                    'title' => Yii::t('yii', 'Ενημέρωση'),
                    'aria-label' => Yii::t('yii', 'Ενημέρωση'),
                    'data-pjax' => '0',
                ], $this->buttonOptions);
        ...
        if (!isset($this->buttons['delete'])) {
            $this->buttons['delete'] = function ($url, $model, $key) {
                $options = array_merge([
                    'title' => Yii::t('yii', 'Διαγραφή'),
                    'aria-label' => Yii::t('yii', 'Διαγραφή'),
                    //'data-confirm' => Yii::t('yii', 'Are you sure you want to delete this item?'),
                    'data-confirm' => Yii::t('yii', 'Είσαι σίγουρος πως θέλεις να κάνεις αυτή τη διαγραφή?'),
                    'data-method' => 'post',
                    'data-pjax' => '0',
                ], $this->buttonOptions);
